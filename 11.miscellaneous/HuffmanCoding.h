...
void HuffmanCoding::transformTreeToArrayOfLists(HuffmanNode *p) { 
    if (p->left == 0 && p->right == 0) { // if p is a leaf,
        ...
        chars[(unsigned char)p->symbol] = p;   // a list associated with symbol found in p;
    }
    else {
...
void HuffmanCoding::encode(ifstream& fIn) {
    ...
    for (fIn.get(ch); !fIn.eof(); ) {
        ...
        if (p->codewordLen < maxPack - packCnt) { // if enough room in
            pack = (pack << p->codewordLen) | p->codeword; // pack to store new codeword
            packCnt += p->codewordLen;  // shift its content to the left and attach new codeword;
        }
        else {      //otherwise move pack's content to the left by the number of left spaces and
            bitsLeft = maxPack - packCnt;
            pack <<= bitsLeft;
            if (bitsLeft != p->codewordLen) {
                hold = p->codeword;
                hold >>= p->codewordLen - bitsLeft;// if new codeword is longer than room left, transfer only as many bits as can fit in pack;
                pack |= hold;
            }
