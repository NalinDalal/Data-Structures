...
public:
  ...
  HuffmanNode(char s, unsigned long f, unsigned int r, HuffmanNode *lt = 0,
              HuffmanNode *rt = 0) {
    symbol = s;
    freq = f;
    runLen = r;
...
void HuffmanCoding::createHuffmanTree() {
  ...
  head->tree = new // initialize list pointers;
      HuffmanNode(data[0].symbol, data[0].freq, data[0].runLen);
  ...
  while (head != tail) { // create one Huffman tree;
    newFreq =
        head->tree->freq + head->next->tree->freq; // two lowest frequencies
...
void HuffmanCoding::createCodewords(HuffmanNode *p, unsigned long codeword,
                                    int level) {
  if (p->left == 0 && p->right == 0) {                  // if p is a leaf,
    ...
    createCodewords(p->right, (codeword << 1) + 1,
                    level + 1); // and 1 for right;
  }
}
...
void HuffmanCoding::transformTreeToArrayOfLists(HuffmanNode *p) {
  if (p->left == 0 && p->right == 0) {          // if p is a leaf,
    ...
    chars[(unsigned char)p->symbol] =
        p; // a list associated with symbol found in p;
  } else {
...
void HuffmanCoding::encode(ifstream &fIn) {
  ...
  for (fIn.get(ch); !fIn.eof();) {
    ...
    if (p->codewordLen < maxPack - packCnt) { // if enough room in
      pack =
          (pack << p->codewordLen) | p->codeword; // pack to store new codeword
      packCnt += p->codewordLen; // shift its content to the left and attach new
                                 // codeword;
    } else { // otherwise move pack's content to the left by the number of left
             // spaces and
      ...
      if (bitsLeft != p->codewordLen) {
        hold = p->codeword;
        hold >>= p->codewordLen -
                 bitsLeft; // if new codeword is longer than room left, transfer
                           // only as many bits as can fit in pack;
        pack |= hold;
      } else
        pack |=
            p->codeword; // if new codeword exactly fits in pack, transfer it;
...
void HuffmanCoding::compress(char *inFileName, ifstream &fIn) {
  ...
  if (strchr(outFileName, '.'))
    ...
    strcat(outFileName, ".z"); // else add extension '.z';
  ...
  cout << "Compression rate = "
       ...
       << 100.0 * (fIn.tellg() - fOut.tellp() + data.size() * (2 + 4)) /
              fIn.tellg();
  fOut.close();
}
  ...
  for (chars = 0, fIn.get(ch); !fIn.eof() && chars < charCnt;) {
    for (HuffmanNode *p = HuffmanTree;;) {
      ...
      if (bitCnt++ == bits) { // read next character from fIn
        ...
        ch <<= 1; // to the left by one position;
    }
  }
}
...
void HuffmanCoding::decompress(char *inFileName, ifstream &fIn) {
  ...
  if (strchr(outFileName, '.'))                  // if there is an extension
    ...
    strcat(outFileName, ".dec"); // else add extension '.z';
  fOut.open(outFileName, ios::out | ios::binary);
